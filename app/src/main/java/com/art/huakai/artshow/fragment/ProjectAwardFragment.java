...
public class ProjectAwardFragment extends BaseFragment {
    ...
    public static class MyHandler extends Handler {
        ...
        private WeakReference<ProjectAwardFragment> reference;

        public MyHandler(ProjectAwardFragment fragment) {
        ...
        public void handleMessage(Message msg) {
            ...
            ProjectAwardFragment fragment = reference.get();
            if (fragment == null) return;
            switch (msg.what) {
                case CODE_COMMIT:
                    String richText = (String) msg.obj;
                    fragment.changeAwardDescription(richText);
                    break;
            }
        }
    }
    ...
    public void initData(@Nullable Bundle bundle) {
        ...
        urlH5 = String.format(
                Constant.URL_REPERTORY_AWARDSDESCPT_WYSIWYG,
                id,
     ...
     * 修改简历个人介绍
     */
    public void changeAwardDescription(final String awardsDes) {
        //判断是否登录
        ...
        if (TextUtils.isEmpty(awardsDes)) {
            Toast.makeText(getContext(), getString(R.string.tip_project_award_input), Toast.LENGTH_SHORT).show();
            return;
        }
        Map<String, String> params = new TreeMap<>();
        ...
        params.put("userId", LocalUserInfo.getInstance().getId());
        params.put("accessToken", LocalUserInfo.getInstance().getAccessToken());
        params.put("awardsDescpt", awardsDes);
        String sign = SignUtil.getSign(params);
        params.put("sign", sign);
        LogUtil.i(TAG, "params = " + params);
        showProgressDialog.show();
        RequestUtil.request(true, Constant.URL_REPERTORY_EDIT_AWARDSDESCPT, params, 82, new RequestUtil.RequestListener() {
            @Override
            public void onSuccess(boolean isSuccess, String obj, int code, int id) {
                ...
                if (isSuccess) {
                    try {
                        showToast(getString(R.string.tip_project_award_input_suc));
                        JSONObject jsonObject = new JSONObject(obj);
                        String projectId = jsonObject.getString("id");
                        ProjectDetailInfo.getInstance().setId(projectId);
                        ProjectDetailInfo.getInstance().setAwardsDescpt(awardsDes);
                        EventBus.getDefault().post(new ProjectInfoChangeEvent());
                        EventBus.getDefault().post(new ProjectNotifyEvent(ProjectNotifyEvent.NOTIFY_AWARD_DES));
                        getActivity().finish();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
